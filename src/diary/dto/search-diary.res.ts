...
export class searchDiaryInfo extends DiaryRes{
  ...
  constructor(diary: Diary, activities:string[] ,emotions: EmotionRes[], targets: string[], search_sentence: string, relate_sentence: string) {
    ...
    if (relateSentenceIndex !== -1) {
      ...
      const endIndex = Math.min(originalContent.length, relateSentenceIndex + relate_sentence.length + 70);
