...
export class AuthController {
  ...
  @ApiOperation({
    summary: '구글 소셜 로그인',
    description:
      '구글 소셜 로그인을 위해서 이 api에 리다이렉트를 걸어주세요. ' +
  ...
  async googleCallback(@Req() req: any, @Res() res: Response) {
    ...
    const redirectUri = decodeURIComponent(
      user.state || this.configService.get('FRONTEND_URL'),
    );

    // ✅ /getaccess 경로 포함해서 리다이렉트 (Access Token + Refresh Token을 쿼리로 전달)
  ...
  @ApiOperation({
    summary: '로그인 테스트',
    description:
      '로그인되어 있다면 200 OK, 그렇지 않다면 401 Unauthorized 반환',
  })
  @Get('test')
  ...
  @ApiOperation({
    summary: '카카오 소셜 로그인',
    description:
      '카카오 소셜 로그인을 위해서 이 api에 리다이렉트를 걸어주세요. ' +
      '로그인이 성공하면 /getaccess?access={jwt} 로 리다이렉트 됩니다. (Refresh Token은 쿠키로 설정됨)',
  })
  @Get('/kakao')
  ...
  async kakaoCallback(
    ...
    const tokens = await this.authService.signInWithKakao(kakaoAuthResCode);

    // ✅ 프론트에서 넘긴 state (redirect_uri)가 user.state에 들어있음
    const redirectUri = decodeURIComponent(
      req.session.state || this.configService.get('FRONTEND_URL'),
    );
  ...
  @ApiOperation({
    summary: '데모 계정 로그인',
    description:
      '데모 계정으로 로그인하여 JWT를 발급받습니다. id를 쿼리스트링으로 받아, 해당 데모 id가 있다면 로그인, 없다면 회원가입합니다',
  })
  @Get('demo')
  async demoLogin(@Query('id') id: string, @Res() res: Response) {
  ...
  @ApiOperation({
    summary: '토큰 갱신',
    description:
      'Authorization 헤더에 있는 Refresh Token을 사용하여 새로운 Access Token을 발급받습니다.',
  })
  @Post('refresh')
